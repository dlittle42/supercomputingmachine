...
(function($){
	...
	EasyFader.prototype = {
		...
		init: function(domNode, settings){
			var self = this;
			console.log('easy init='+$(domNode).attr('id'));
			if(settings){
		...
		handlers: {
			default: function(){
				...
				self.$container.find('.pager').on('click',function(){
				 ...
				 self.$container.on('pause.easyFader', function(){
				 	console.log('pause.easyFader');
				 	self.pause();
				 });
				 self.$container.on('play.easyFader', function(){
				 	console.log('play.easyFader');
				 	self.play();
				 });
				 /*
		...
		fadeSlides: function(activeNdx, newNdx){
			...
			self.$slides.eq(activeNdx).css('z-index', 1002);
			self.$slides.eq(newNdx).css('z-index', 1003);
			if(!self.prefix){
